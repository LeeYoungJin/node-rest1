...
export class CommentController {
  static addComment = async (req, res) => {
    ...
    const board = await AppDataSource.getRepository(Board).findOneBy({
      id: board_id,
    });
  ...
  static findAllComment = async (req, res) => {
    ...
    const comments = await AppDataSource.getRepository(Comment).find(options);
    res.send(comments);
  };
  // board의 id에 해당하는 모든 comment 갯수
  static countBoardComment = async (req, res) => {
    const { board_id } = req.query;
    const total = await AppDataSource.getRepository(Comment).find({
  ...
  static findOneComment = async (req, res) => {
    ...
    const comment = await AppDataSource.getRepository(Comment).findOneBy({
      id,
    });
    console.log(comment);
    res.send(comment);
